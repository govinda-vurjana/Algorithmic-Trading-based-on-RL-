...
def predict_trade(data_path: str) -> dict:
    ...
    if len(strategy_returns) > 0 and np.any(strategy_returns != 0):
        cumulative_returns = np.expm1(np.sum(np.log1p(np.abs(strategy_returns) + 1e-8) * np.sign(strategy_returns)))
    ...
    cumulative_returns = max(-0.99, min(10.0, cumulative_returns))
